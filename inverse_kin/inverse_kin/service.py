...
class Service(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(
            GoToPosition, "jint_control_service", self.interpolation_service_callback
        )
    ...
    def interpolate_linear(self, end_pos, time, T, iteration, start_pos=0) -> float:
        return start_pos + (end_pos - start_pos) / time * T * iteration

    def interpolate_polynomial(self, end_pos, time, T, iteration, start_pos=0) -> float:
    ...
    def get_item_position(self) -> PyKDL.Frame:
        frame1 = PyKDL.Frame(
            ...
            PyKDL.Vector(0, 0, self.d + self.box_height + self.cylinder_radius),
        )
        frame2 = PyKDL.Frame(
    ...
    def interpolation_service_callback(self, req, res):
        ...
        end_positions = [req.translation, req.first_rotation, req.second_rotation]
        steps = int(req.time / T) + 1
        for k in range(1, steps):
            if req.interpolation_type == "linear":
                positions = [
                    self.interpolate_linear(end, req.time, T, k)
                    for end in end_positions
                ]
            elif req.interpolation_type == "polynomial":
                positions = [
                    self.interpolate_polynomial(end, req.time, T, k)
                    for end in end_positions
                ]
            else:
