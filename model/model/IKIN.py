...
class IKIN(Node):
    ...
    def listener_callback(self, msg):
        msg=msg.pose
        if msg.position.z <= (self.d_start+self.cylinder_radius+self.box_height) and msg.position.z >= (self.cylinder_radius+self.box_height):
            ...
            if len(params)!=0:
                ...
                self.get_logger().warn('Zła odleglosc od robota - nie można określić położenia stawów')
                self.d = 2.0
                self.theta1=1.0
                self.theta2=1.0
        else:
            self.get_logger().warn("Zła wysokość - nie można określić położenia stawów"+(math.sqrt((msg.position.x)**2 + (msg.position.y)**2)))
            self.d = 2.0
            self.theta1=1.0
            self.theta2=1.0
        ...
        self.joint_publisher = self.create_publisher(JointState, 'joint_states', QoSProfile(depth=10))
